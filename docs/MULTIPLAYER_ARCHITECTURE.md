...
- Deliberate and load-bearing: a reaction claim or pass does **not** bump the version. All eligible seats therefore submit against the same version concurrently, and `respondedSeats` — not the version — prevents a double answer. Preserve this exactly.
  ...
  This rule has a consequence that cost a real bug. A transport that decides when to broadcast by watching for public change will never broadcast a reaction answer: the version does not move, and a seatless viewer never held the legal actions that just disappeared, so the spectator view is byte-identical. The seat that just answered is then left rendering a live Pon or Ron button that the room now rejects with `ALREADY_RESPONDED`, and the other eligible seats are told nothing while they are still being waited on. Any change signal must therefore include the barrier's answered set, not just the public view — and it must do so *without* bumping the version, which would break the barrier itself.
